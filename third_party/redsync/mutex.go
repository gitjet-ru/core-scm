...
func (m *Mutex) actOnPoolsAsync(actFn func(redis.Pool) (bool, error)) (int, error) {
	...
	for range m.pools {
		...
		if r.statusOK {
			...
			err = appendErr(err, ErrLockAlreadyExpired)
		} else if r.err != nil {
			err = appendErr(err, &RedisError{Node: r.node, Err: r.err})
		} else {
			taken = append(taken, r.node)
			err = appendErr(err, &ErrNodeTaken{Node: r.node})
		}
...
func appendErr(base error, next error) error {
	if base == nil {
		return next
	}
	if next == nil {
